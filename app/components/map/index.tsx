...
const MapComponent = ({config, loadedCallback} : {config: any, loadedCallback: any}) => {
  ...
  const displayFeatureInfo = (evt: MapBrowserEvent<UIEvent>, dimensions: any[]) => {
    ...
    if (features.length > 0) {
      ...
      tooltipElement.current!.children[0]!.innerHTML = features[0].get(config.legendConcept)
  ...
  useEffect(() => {
    ...
    sdmxParser.getDatasets(dataFlowUrl, {
          ...
          loader: (extent, resolution, projection, success, failure) => {
            ...
            fetch(url).then((response) => response.json()).then((response) => {
              ...
              geoDimension.values.forEach((geoValue: any) => {
                const feature = features?.find((feat: FeatureLike, index: number, features: FeatureLike[]) => feat.getProperties()[dataObj.geojsonKey] == geoValue['id'])
      ...
      const vectorProjection = geoJSONFormat.readProjection(vectorSource)
